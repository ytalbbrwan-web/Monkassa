...
OFFICE_DELIVERY_DEFAULT = 50 OFFICE_DELIVERY_SOUTH = 120 OFFICE_DELIVERY_FREE = ["وهران", "oran"]

================== REGIONS ==================

def get_region(wilaya): wilaya = wilaya.lower() east = ["سطيف","عنابة","قسنطينة","جيجل","سكيكدة","باتنة","تبسة","خنشلة","الطارف","سوق اهراس"] west = ["وهران","تلمسان","سيدي بلعباس","معسكر","غليزان","البيض","النعامة","عين تموشنت"] center = ["الجزائر","البليدة","تيبازة","بومرداس","المدية","عين الدفلى","الشلف","تيزي وزو","البويرة"] south = ["أدرار","تمنراست","إليزي","تندوف","بشار","غرداية","ورقلة","الأغواط","الوادي"] if wilaya in east: return "east" if wilaya in west: return "west" if wilaya in center: return "center" if wilaya in south: return "south" return "center"
...
def ai_reply(text): if not OPENAI_API_KEY: return "مرحبا 👋 كيف نقدر نعاونك؟" headers = {"Authorization": f"Bearer {OPENAI_API_KEY}", "Content-Type": "application/json"} data = { "model": "gpt-4.1-mini", "messages": [ {"role": "system", "content": "انت بائعة جزائرية في متجر أحذية نسائية Monkassa تقنعي الزبونة بالشراء باختصار"}, {"role": "user", "content": text} ] } r = requests.post("https://api.openai.com/v1/chat/completions", headers=headers, json=data) try: return r.json()["choices"][0]["message"]["content"] except: return "مرحبا 🌸 تحبي تعرفي السعر ولا التوصيل؟"
...
# DELIVERY REQUEST
if any(word in text_lower for word in ["توصيل","delivery"]):
...
🏠 للمنزل: {home} دج 🏢 للمكتب: {office} دج") return "ok"
